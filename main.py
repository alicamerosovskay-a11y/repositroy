...
# Удаляем старые сессии при старте (если есть)
...
async def start(c, m):
    ...
    if chat_id in user_states and user_states[chat_id].get("logged"):
        await m.reply("✅ Уже в системе. /spam @username")
        return
...
async def get_number(c, m):
    ...
    try:
        ...
        await m.reply(f"✅ Код отправлен на {number}. Введи цифры:")
...
async def get_number(c, m):
    ...
    number = m.text.strip()
    try:
        # НЕ используем in_memory — создаём файл сессии, но с полным путём
        session_path = f"sessions/user_{chat_id}"
        client = Client(
            session_path,
            api_id=API_ID,
            api_hash=API_HASH
        )
        # Запускаем клиент БЕЗ интерактивного ввода
        await client.start()
